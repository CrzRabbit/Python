...
class Solution:
    ...
    '''
    @printTime()
    def _1findLadders(self, beginWord: str, endWord: str, wordList: List[str]) -> List[List[str]]:
        if endWord not in wordList:
            return []
        wordList = [beginWord] + wordList
        n = len(wordList)
        ...
        def merge(i, j):
            ...
            if rank[i] == rank[j] and i != j:
                rank[i] += 1
        for i in range(n):
            if i != 0 and same(wordList[0], wordList[i]):
                merge(0, i)
            if i != 1 and same(wordList[1], wordList[i]):
                merge(1, i)
        for i in range(n):
            for j in range(i + 1, n):
                if same(wordList[i], wordList[j]):
...
Solution()._1findLadders(beginWord, endWord, wordList)
'''
